    ...
    class Map
    {
        ...
        internal void Update(GameTime gameTime)
        {
            ...
            if (lastCloudSpawned >= 3000)
            {
                ...
                {
                    cloudsList.Add(new Cloud(new Vector2(localPlayer.Position.X + 600, rnd.Next(200, 300)), rnd.Next(0, 4),(float)(rnd.NextDouble()+1),rnd.Next(1,3)));
                }
            }
            foreach (var cloud in cloudsList.ToList())
            {
                cloud.Update(gameTime);
                if (Vector2.Distance(cloud.Position, localPlayer.Position) > 3000)
